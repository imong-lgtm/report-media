...
@section('content')
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        <div
            class="px-8 py-8 border-b border-gray-50 flex justify-between items-center bg-gradient-to-r from-white to-gray-50/50">
            <div>
            ...
            <a href="{{ route('admin.articles.create') }}"
                class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl font-bold transition-all shadow-lg shadow-blue-100 hover:-translate-y-0.5">
        ...
        @if(session('success'))
            <div
                class="m-8 p-4 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-2xl flex items-center gap-3 animate-fade-in">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    @else
                                        <div
                                            class="h-12 w-12 bg-gray-100 rounded-xl border border-dashed border-gray-200 flex items-center justify-center text-gray-400">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-xs text-gray-400 font-medium italic">Oleh
                            ...
                            <td class="px-8 py-6 text-gray-500 font-medium">
                                <span
                                    class="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-xs font-bold">{{ $article->category->name }}</span>
                            </td>
                            <td class="px-8 py-6">
                                @if($article->status === 'published')
                                    <span
                                        class="px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-xs font-bold uppercase">Published</span>
                                @else
                                    <span
                                        class="px-2 py-1 bg-amber-50 text-amber-600 rounded-lg text-xs font-bold uppercase">Draft</span>
                                @endif
                            </td>
