        ...
        <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ $article->title }}" class="w-full px-6 py-4 rounded-2xl border border-gray-100 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all outline-none bg-gray-50/50 font-bold text-lg" placeholder="Masukkan judul berita..." required>
                </div>
            ...
            <div>
                ...
                @if($article->image)
                    <div class="mb-4">
                        <img src="{{ asset('storage/' . $article->image) }}" class="h-32 w-48 rounded-2xl object-cover border border-gray-100">
                    </div>
                @endif
            ...
            <div>
                ...
                <textarea name="content" rows="15" class="w-full px-6 py-6 rounded-[2rem] border border-gray-100 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all outline-none bg-gray-50/50 font-medium leading-relaxed" required>{{ $article->content }}</textarea>
            </div>
