...
<div class="max-w-2xl bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
    <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ $service->title }}" class="w-full px-4 py-3 rounded-2xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition outline-none" required>
        </div>
        <div>
            <label class="block text-sm font-bold text-gray-700 mb-2">Service Image</label>
            @if($service->image)
                <img src="{{ $service->image }}" class="h-20 w-20 rounded-xl object-cover mb-4">
            @endif
        ...
        <div>
            ...
            <textarea name="description" rows="5" class="w-full px-4 py-3 rounded-2xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition outline-none" required>{{ $service->description }}</textarea>
        </div>
