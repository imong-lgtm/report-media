...
<div class="mb-8 flex justify-between items-center">
    ...
    <a href="{{ route('admin.users.create') }}" class="px-6 py-3 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-600/20 flex items-center gap-2">
...
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden text-sm md:text-base">
    <div class="overflow-x-auto">
        <table class="w-full text-left">
            <thead class="bg-gray-50 text-gray-500 text-xs uppercase font-bold tracking-widest border-b border-gray-100">
                <tr>
                ...
                <tr class="hover:bg-gray-50/50 transition">
                    <td class="px-8 py-6">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                @if($user->id === auth()->id())
                                    <span class="text-[10px] bg-blue-600 text-white px-2 py-0.5 rounded-full font-bold">YOU</span>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td class="px-8 py-6">
                        <span class="px-3 py-1 rounded-full text-xs font-bold uppercase {{ $user->role === 'superadmin' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600' }}">
                            {{ $user->role }}
                        </span>
                    </td>
                    ...
                    <td class="px-8 py-6 text-right">
                        ...
                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="p-2 text-red-400 hover:text-red-600 transition" onclick="return confirm('Remove admin access for {{ $user->name }}?')">
